...
Page({
	  ...
	  var that = this;
	    wx.request({
	        ...
	        success: function (res) {
                ...
                //处理佣金(仅求购求租委托才有)
                if (!!info.entrustUser.broberComitionRage){
                  switch (info.entrustUser.caseType){
                    case '4':
                      //求租委托
                      info.entrustUser['broberComitionRageNum'] = info.entrustUser.broberComitionRage.substr(0, 2);//佣金数量
                      info.entrustUser['broberComitionRageUnit'] = info.entrustUser.broberComitionRage.substr(-3, 3);//佣金单位
                      break;
                    case '3':
                      //求购委托
                      var broberComitionRageList = info.entrustUser.broberComitionRage.split("%")
                      info.entrustUser['broberComitionRageNum'] = broberComitionRageList[0]
                      info.entrustUser['broberComitionRageUnit'] = info.entrustUser.broberComitionRage.substr(-1, 1);//佣金单位
                      break;
                  }
                }
  ...
  imContact:function(e){
    var that = this;
      ...
      unreadMsg = common.removeUnreadNum('', that.data.entrustUser.brokerArchiveId);
		  wx.navigateTo({
  			url: url
		  })
	  }

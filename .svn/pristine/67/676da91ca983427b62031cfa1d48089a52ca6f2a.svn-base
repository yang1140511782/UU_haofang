 ...
 */
   ...
   * 裁剪图片
   */
  addImgParamCrop(url,w,h){
	  if(!url){
		  return "";
	  }
	  url=url.replace("http://pic.myfun7.com/http://pic.myfun7.com/","http://pic.myfun7.com/");
	  if(url.indexOf("oss") == -1){
		  if(url.indexOf("group")){
			  url = url.replace("pic.myfun7.com","picbj.myfun7.com");
		  }
		  url += "."+w+"x"+h+".jpg";
	  }else{
		  url=url.replace("/pic/hftpic/house/","/");
  ...
  addImgParam(url,w,h){
	  ...
	  if(url.indexOf("oss") == -1){
		  ...
		  url=url.replace("/pic/hftpic/house/","/");
		  url +="?x-oss-process=image/resize,m_mfit,h_"+h+",w_"+w;
	  }
	  return url;
  }
  isIOS(){
 ...
 */
getBehaviorConfig(behaviorType){
...
getSaveCustBehaviorPage(type){
  ...
  if(pages.length > 0){
      for (var i = pages.length - 1; i >= 0; i--) {
        ...
        if(typeof(pages[i].saveCustBehaviorForStore) != 'undefined' && pages[i].route==page){
          console.log(behaviorType);
          pages[i].saveCustBehaviorForStore(behaviorType);
          break;
        }
      }
  }else{

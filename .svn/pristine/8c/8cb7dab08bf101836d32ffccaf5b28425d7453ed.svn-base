...
Page({
  ...
  onLoad: function (options) {
    ...
    if(locateRegId > 0 && wx.getStorageSync('locateCityId') == wx.getStorageSync('cityId')){
      this.rejectAjaxData('regionId', locateRegId);//修改请求条件:设置区域
    }
  ...
  getListValue(blooean, firstIn) {
    ...
    api.getList(url, params).then(res => {
      ...
      //提前请求下一页接口问题 , 以提高下一次访问接口的速度
      var paramsNewStr =  JSON.stringify(params);
      var paramsNew = JSON.parse(paramsNewStr);
      paramsNew.pageNum++;
      ...
      if (res.DATA.trueHouseNum != 0){
        wx.showToast({
          title: '共' + res.DATA.trueHouseNum + '套房源',
          icon: 'none',
        })
      }
      res = res.DATA.list;
      res.map(function (ele, i) {
        if (ele['houseTagDesc']) {
          ele['houseTagDesc'] = ele['houseTagDesc'].split('|').slice(0, 4);
        } else if (ele.projectSpec) {
          if (ele['projectSpec'].indexOf('、') > -1) {
            ele['houseTagDesc'] = ele['projectSpec'].split('、').slice(0, 4);
          } else {
            ele['houseTagDesc'] = ele['projectSpec'].split(',').slice(0, 4);
          };
        } else if (ele['buildType']) {
            ele['houseTagDesc'] = ele['buildType'].split(',').slice(0, 4);
            if (ele['buildFitment']) {
      ...
      if (res.length < _this.data.pageSize && res.length > 0 || res.length == 0 && _this.data.ajaxListData.pageNum > 1) {
        _this.setData({
          ajaxListTag: false,
          loadingdata: false,
          noMoreData: true
  ...
  goToPerStore: function (e) {
    var url = "/pages/personalStore/personalStore?scene=" + this.data.buildExpert.buildOwnerArchiveId
    wx.navigateTo({
      url: url
    })
  }

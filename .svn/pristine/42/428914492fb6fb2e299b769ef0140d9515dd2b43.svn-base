...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      ...
      caseType:options.caseType || options.casetype 
    });
    ...
    if (typeof (options.caseType) != 'undefined'){
      that.rejectAjaxData('caseType', options.caseType);
      if(options.caseType == 3){
        //设置合租的页面标题
  ...
  moreReset() {
    ...
    if (ajaxListData.caseType == 3){
      var obj = this.data.moreResetObj;
      obj['caseType'] = '3';
      this.setData(obj);
      this.rejectAjaxData('area', '');
    }else{
      var obj = this.data.moreResetObj;
      obj['caseType'] = '2';
      this.setData(obj);
  ...
  mapBtnClick() {
	  wx.navigateTo({
	    url: '/packageTool/pages/findHouseByMap/findHouseByMap?caseType=2'
	  });
  },
  /**
   * 点击排序蒙层,关闭排序弹框
  ...
  getListValue(blooean, firstIn) {
    ...
    api.getList(url, params).then(res => {
      ...
      res.map(function (ele, i) {
	    ...
	    if(!ele['caseType']){
	    	ele['caseType'] = _this.data.caseType;
	    }
      });
      if (true) {
        //对列表的赋值  
  ...
  goToEntrustLiBtn: function (e) {
    var _this = this;
     ...
     var _buildOwnerArchiveId = e.currentTarget.dataset.buildownerarchiveid;
    var _userPhone = _this.data.userPhone;
    var goToUrl = "/pages/registration/registration?caseType=2&archiveId=" + _buildOwnerArchiveId + '&isVip=1';
    wx.navigateTo({
    	url: goToUrl,
    });
  },
